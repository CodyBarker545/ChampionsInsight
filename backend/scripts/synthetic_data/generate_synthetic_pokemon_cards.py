"""
Generate synthetic Pok̩mon slot-card training images from clean Champions menu sprites.

This version:
- Reads both normal and shiny folders.
- Merges shiny with normal as the same class, while tracking each variant separately.
- Ignores Mega forms.
- Creates red/pink slot-card style images.
- Uses no meaningful tilt.
- Focuses on scale, shifting, partial cutoffs, blur, noise, glare, scanlines, exposure,
  and JPEG artifacts while keeping enough sprite identity visible to remain legible.
...
MIN_VISIBLE_SPRITE_PIXELS = 420
MIN_VISIBLE_SPRITE_RATIO = 0.08
MIN_VISIBLE_SPRITE_BBOX_SIZE = 20
MIN_RETAINED_SPRITE_RATIO = 0.65
...
def parse_menu_cp_from_name(name: str) -> tuple[int | None, str | None]:
    ...
    if not match:
        return None, None
    return int(match.group(1)), normalize_label(match.group(2)) if match.group(2) else None
...
def load_pokemon_mapping() -> tuple[dict[tuple[int, str | None], str], dict[int, str]]:
    ...
    for dex_key, forms in lookup.items():
        ...
        for form in forms:
            ...
            if not form_api_name or "mega" in form_api_name.split("_"):
                continue
            ...
            suffixes = {form_api_name, form_display}
            if species_api_name and form_api_name.startswith(species_api_name + "_"):
                suffixes.add(form_api_name[len(species_api_name) + 1 :])
            for suffix in filter(None, suffixes):
                mapping[(dex_number, normalize_label(suffix))] = form_api_name

    return mapping, base_by_dex


POKEMON_LABEL_MAPPING, BASE_LABEL_BY_DEX = load_pokemon_mapping()
...
def parse_label(path: Path) -> str | None:
    ...
    if "mega" in path.stem.lower():
        return None

    dex_number, form_suffix = parse_menu_cp_from_name(path.name)
    if dex_number is not None:
        label = POKEMON_LABEL_MAPPING.get((dex_number, form_suffix))
        if not label and form_suffix and dex_number in BASE_LABEL_BY_DEX:
            label = f"{BASE_LABEL_BY_DEX[dex_number]}_{form_suffix}"
        return label or POKEMON_LABEL_MAPPING.get((dex_number, None)) or BASE_LABEL_BY_DEX.get(dex_number)

    stem = path.stem.replace("_shiny", "").replace("-shiny", "").replace(" shiny", "")
...
def has_enough_visible_sprite(mask: Image.Image, original_sprite_pixels: int) -> bool:
    arr = np.array(mask)
    ...
    if visible_pixels / max(1, original_sprite_pixels) < MIN_RETAINED_SPRITE_RATIO:
        return False
...
def preserves_sprite_shape(
    crop_mask: Image.Image,
    original_bbox_size: tuple[int, int],
) -> bool:
    bbox = crop_mask.getbbox()
    if not bbox:
        return False
    left, top, right, bottom = bbox
    original_width, original_height = original_bbox_size
    return (
        (right - left) / max(1, original_width) >= 0.58
        and (bottom - top) / max(1, original_height) >= 0.58
    )
...
def crop_real_like_from_card(
    ...
    for _ in range(MAX_CROP_ATTEMPTS):
        ...
        if (
            ...
            and
            has_enough_visible_sprite(crop_mask, original_sprite_pixels)
            and preserves_sprite_shape(crop_mask, original_bbox_size)
        ):
            break
    else:
...
def add_camera_effects(img: Image.Image) -> Image.Image:
    ...
    img = ImageEnhance.Brightness(img).enhance(random.uniform(0.58, 1.42))
    img = ImageEnhance.Contrast(img).enhance(random.uniform(0.68, 1.45))
    img = ImageEnhance.Color(img).enhance(random.uniform(0.72, 1.35))

    if random.random() < 0.55:
        img = img.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.18, 1.35)))
    ...
    if random.random() < 0.22:
        # Horizontal scanlines from poor display capture / rolling shutter.
        line_strength = random.randint(3, 12)
        every = random.randint(2, 5)
        arr[::every, :, :] -= line_strength

    if random.random() < 0.18:
        # Very mild color cast from phone white balance.
        cast = np.array(
            [
                random.randint(-12, 16),
                random.randint(-10, 10),
                random.randint(-16, 12),
            ],
            dtype=np.int16,
        )
        arr += cast[None, None, :]
...
def paste_sprite_on_card(sprite_path: Path) -> Image.Image:
    ...
    max_sprite_w = int(WORK_CARD_WIDTH * random.uniform(0.40, 1.28))
    max_sprite_h = int(WORK_CARD_HEIGHT * random.uniform(0.42, 1.28))

    scale = min(max_sprite_w / sprite.width, max_sprite_h / sprite.height)
    scale *= random.uniform(0.86, 1.18)
    ...
    original_sprite_pixels = int(np.count_nonzero(np.array(sprite.getchannel("A")) > 24))
    alpha_bbox = sprite.getchannel("A").getbbox()
    original_bbox_size = (
        max(1, (alpha_bbox[2] - alpha_bbox[0]) if alpha_bbox else sprite.width),
        max(1, (alpha_bbox[3] - alpha_bbox[1]) if alpha_bbox else sprite.height),
    )
    ...
    min_x = -int(sprite.width * 0.34)
    max_x = WORK_CARD_WIDTH - int(sprite.width * 0.66)

    min_y = -int(sprite.height * 0.30)
    max_y = WORK_CARD_HEIGHT - int(sprite.height * 0.70)
    ...
    if random.random() < 0.72:
        img, _ = crop_real_like_from_card(
            img,
            sprite_mask,
            original_sprite_pixels,
            original_bbox_size,
        )
    else:
...
def collect_sprite_paths(input_dir: Path) -> list[Path]:
    ...
    return sorted(
        [path for path in sprite_paths if parse_label(path) is not None],
        key=lambda path: (
            parse_label(path) or path.stem,
            "_shiny" in path.stem.lower(),
            path.name.lower(),
        ),
    )
...
def generate_dataset(
    input_dir: Path,
    output_dir: Path,
    images_per_sprite: int | None,
    target_per_variant: int | None,
    seed: int,
    start_index: int,
    ...
    if target_per_variant is None and images_per_sprite is None:
        raise ValueError("Provide --images-per-sprite or --target-per-variant.")
    ...
    if start_index or max_sprites is not None:
        end_index = None if max_sprites is None else start_index + max_sprites
        sprite_paths = sprite_paths[start_index:end_index]
        print(f"Processing sprite slice: start={start_index}, count={len(sprite_paths)}")
    if target_per_variant is not None:
        print(f"Target per variant: {target_per_variant}")
    else:
    ...
    for index, sprite_path in enumerate(sprite_paths, start=1):
        ...
        existing_variant_count = len(list(label_dir.glob(f"{sprite_path.stem}_synthetic_*.jpg")))
        images_to_write = (
            max(0, target_per_variant - existing_variant_count)
            if target_per_variant is not None
            else int(images_per_sprite or 0)
        )

        if images_to_write == 0:
            print(f"  already at target: {existing_variant_count}")
            continue

        for offset in range(images_to_write):
            try:
                img = paste_sprite_on_card(sprite_path)

                i = existing_variant_count + offset
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Top each normal/shiny source sprite up to this many images instead of always appending.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Start at this sprite index after label/variant sorting.",
    )
    parser.add_argument(
        "--max-sprites",
        type=int,
        default=None,
        help="Process at most this many sprite files in this run.",
    )
    ...
    generate_dataset(
        ...
        target_per_variant=args.target_per_variant,
        seed=args.seed,
