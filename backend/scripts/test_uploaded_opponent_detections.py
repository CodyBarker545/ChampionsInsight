...
def analyze_slot(slot):
    ...
    if not pokemon_name:
        issues.append("No Pokémon name returned.")
        recommendations.append("Check card crop placement and whether the Pokémon crop contains the sprite/card image.")
    ...
    if confidence is None:
        ...
        strengths.append("Pokémon confidence is strong.")
    elif confidence >= OK_CONFIDENCE:
        issues.append("Pokémon confidence is usable but not strong.")
        recommendations.append("Use top-candidate gap and type filtering before accepting this prediction.")
    else:
        issues.append("Pokémon confidence is weak.")
        recommendations.append("Run type-combo fallback and inspect the Pokémon crop for blur, glare, or bad placement.")

    if candidate_gap is None:
        issues.append("Top candidate gap unavailable.")
        recommendations.append("Return top 5 Pokémon candidates from the detector so weak/confusing matches can be analyzed.")
        ...
        recommendations.append("Treat this as uncertain. The crop likely matches multiple Pokémon too closely.")
    ...
    if not crop_path:
        issues.append("No debug Pokémon crop path returned.")
        recommendations.append("Save Pokémon crop images for every slot. This is required to debug placement.")
    else:
        strengths.append("Debug Pokémon crop was saved.")
    ...
    if detected_types and reference_types:
        ...
        issues.append("Types were detected, but Pokémon reference types are missing.")
        recommendations.append("Load known Pokémon types locally so type detection can be used for candidate filtering.")
    elif reference_types and not detected_types:
        recommendations.append("Type detection was skipped or failed. This is okay if Pokémon confidence/gap is strong.")
...
def summarize_image_result(filename, result, elapsed_ms):
    ...
    if result.get("status") != "success":
        ...
        image_notes.append("Some slots returned no Pokémon name.")
...
def build_overall_summary(results):
    ...
    return {
        ...
        "debugInterpretation": [
            ...
            "If acceptedFastPathSlotCount is high, Pokémon image matching is strong enough for many slots.",
