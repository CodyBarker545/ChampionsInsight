...
def crop_with_visible_center(image: Image.Image) -> Image.Image:
    ...
    crop_width = int(round(width * random.uniform(0.72, 1.0)))
    crop_height = int(round(height * random.uniform(0.72, 1.0)))
...
def add_camera_effects(image: Image.Image) -> Image.Image:
    ...
    image = ImageEnhance.Contrast(image).enhance(random.uniform(0.82, 1.32))
    image = ImageEnhance.Color(image).enhance(random.uniform(0.84, 1.22))
