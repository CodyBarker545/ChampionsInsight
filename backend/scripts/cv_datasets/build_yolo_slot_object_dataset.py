...
This is the Stage 2 detector dataset:
    red slot crop -> pokemon_sprite and individual type_icon boxes
...
def prepare_pokemon_label_box(
    ...
    """
    Prefer the broader pre-trim proposal when available, then pad it.

    The old auto-label path used the already-trimmed sprite box, which can hug
    Incineroar-like sprites too tightly when dark limbs blend into the card.
    For YOLO training we want the whole visible spirit, not only the easiest
    foreground pixels, so use rawBox as the starting point when it exists.
    """
    raw_box = detected_object.get("rawBox") or get_object_box(detected_object)
    box = clamp_object_box(raw_box, image_width, image_height)
    return expand_labeled_box(
        box,
        image_width,
        image_height,
        x_ratio,
...
def prepare_type_label_box(
    raw_box: dict,
    image_width: int,
    image_height: int,
    square_padding: float,
) -> dict:
    """
    Force type labels to clean square tiles with light padding.

    Type icons are UI tiles, not free-form objects. Training YOLO on square,
    conservative boxes is cleaner than inheriting slightly stretched boxes
    from heuristics or older detections.
    """
    box = clamp_object_box(raw_box, image_width, image_height)
    return cv_service.normalize_type_box(
        box,
        image_width,
        image_height,
        padding_ratio=square_padding,
    )
...
def object_quality_notes(label: str, box: dict, image_width: int, image_height: int) -> list[str]:
    ...
    if label == "type_icon":
        ...
        if width_ratio < 0.08 or height_ratio < 0.16:
            notes.append("type_icon_too_small")
        if width_ratio > 0.36 or height_ratio > 0.72:
            notes.append("type_icon_too_large")
        if aspect < 0.45 or aspect > 1.80:
            notes.append("type_icon_bad_aspect")
...
def collect_slot_objects(
    ...
    if type_icon_crops:
        for icon_crop in type_icon_crops:
            raw_box = {
                "x": icon_crop.get("x", 0),
                "y": icon_crop.get("y", 0),
            ...
            box = prepare_type_label_box(
                raw_box,
                image_width,
                image_height,
                type_square_padding,
            )
            object_notes = object_quality_notes("type_icon", box, image_width, image_height)
            objects.append(
                {
                    "classId": 1,
                    "className": "type_icon",
                    "box": box,
                    "source": icon_crop.get("source", icon_crop.get("cropSource", "")),
        ...
        notes.append("missing_type_icon")
...
def build_dataset(args: argparse.Namespace) -> None:
    ...
    print(f"Type icons:       {type_icon_count}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build YOLO data for Pokemon and individual type-icon objects inside red slot crops."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.04,
        help="Padding ratio applied after forcing each type icon box to a square tile.",
    )
    parser.add_argument(
        "--include-review",
        action="store_true",
