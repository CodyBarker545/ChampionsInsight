"""Create training folders for red slot PokÃ©mon crops and type combo crops.
...
def slugify(value):
    ...
    value = value.replace("â™€", "f").replace("â™‚", "m")
    value = value.replace("'", "")
    ...
    for name in sorted(names, key=slugify):
        ...
        if not readme_path.exists():
            readme_path.write_text(
                (
                    f"Label: {name}\n\n"
                    "Put full red opponent slot crops for this PokÃ©mon here.\n"
                    "Example image: opponent-slot-1.jpg\n\n"
                    "The image should include the PokÃ©mon, red card background, "
        ...
        if not readme_path.exists():
            readme_path.write_text(
                (
                    ...
                    "The crop should show the full type icon area, not the full PokÃ©mon slot.\n"
                    "Only use manually verified examples.\n"
                ),
                encoding="utf-8",
            )
...
def create_root_readme():
    ...
    (OUTPUT_ROOT / "_README.txt").write_text(
        (
            ...
            "=================================\n\n"
            "Use slot_pokemon/ for full red slot crops labeled by PokÃ©mon.\n"
...
def main():
    ...
    print(f"Slot PokÃ©mon folders: {pokemon_count}")
